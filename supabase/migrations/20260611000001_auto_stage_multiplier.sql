...
SET stage_multiplier = CASE
    WHEN round ILIKE '%Final%' AND round NOT ILIKE '%Quarta%' AND round NOT ILIKE '%Oitava%' AND round NOT ILIKE '%Semi%' THEN 2.5
    WHEN round ILIKE '%Semi%' THEN 2.0
    WHEN round ILIKE '%Quarta%' THEN 2.0
    WHEN round ILIKE '%Oitava%' THEN 1.5
    ELSE 1.0
END;
...
BEGIN
    NEW.stage_multiplier := CASE
        WHEN NEW.round ILIKE '%Final%' AND NEW.round NOT ILIKE '%Quarta%' AND NEW.round NOT ILIKE '%Oitava%' AND NEW.round NOT ILIKE '%Semi%' THEN 2.5
        WHEN NEW.round ILIKE '%Semi%' THEN 2.0
        WHEN NEW.round ILIKE '%Quarta%' THEN 2.0
        WHEN NEW.round ILIKE '%Oitava%' THEN 1.5
        ELSE 1.0
    END;
    RETURN NEW;
END;
